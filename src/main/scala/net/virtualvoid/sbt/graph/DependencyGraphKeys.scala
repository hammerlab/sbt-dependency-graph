...
trait DependencyGraphKeys {
  ...
  val dependencyBrowseTreeTarget = SettingKey[File]("dependency-browse-tree-target",
    "The location dependency browse tree files should be put.")
  val dependencyBrowseTreeHTML = TaskKey[URI]("dependency-browse-tree-html",
    "Creates an HTML page that can be used to view the dependency tree")
  val dependencyBrowseTree = TaskKey[URI]("dependency-browse-tree",
